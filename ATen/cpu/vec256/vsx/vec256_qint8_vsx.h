...
struct Vec256<c10::qint8> {
 ...
 public:
  ...
  __inline_attrs Vec256(__vchari v) : _vec0{v}, _vec1{v} {}
  ...
  static __inline_attrs Vec256<c10::qint8> loadu(
      const void* ptr,
      int count = size()) {
    if (count == size()) {
      return {
 ...
 public:
  ...
  Vec256<c10::qint8> __inline_attrs relu(Vec256<c10::qint8> zero_point) const {
    return {vec_max(_vec0, zero_point._vec0), vec_max(_vec1, zero_point._vec1)};
  }

  Vec256<c10::qint8> __inline_attrs
  relu6(Vec256<c10::qint8> zero_point, Vec256<c10::qint8> q_six) const {
    __vchari max0 = vec_max(_vec0, zero_point._vec0);
    __vchari max1 = vec_max(_vec1, zero_point._vec1);
    return {vec_min(max0, q_six._vec0), vec_min(max1, q_six._vec1)};
  }

  int_vec_return_type widening_subtract(Vec256<c10::qint8> b) const {
    __vshi vecshi0 = vec_unpackh(_vec0);
    __vshi vecBshi0 = vec_unpackh(b._vec0);
    __vshi vecshi1 = vec_unpackl(_vec0);
    __vshi vecBshi1 = vec_unpackl(b._vec0);

    __vshi vecshi2 = vec_unpackh(_vec1);
    __vshi vecBshi2 = vec_unpackh(b._vec1);
    __vshi vecshi3 = vec_unpackl(_vec1);
    __vshi vecBshi3 = vec_unpackl(b._vec1);
...
template <>
Vec256<c10::qint8> inline minimum(
    const Vec256<c10::qint8>& a,
    const Vec256<c10::qint8>& b) {
  return a.minimum(b);
}
